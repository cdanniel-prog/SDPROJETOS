...
//Função para salvar o novo usuário no arquivo
    FILE *arquivo = fopen(nomeArquivo, "a");
    if(arquivo != NULL){
        fprintf(arquivo, "%s, %s, %s, %.2f\n", usuario->nome, usuario->email, usuario->senha, usuario->saldo);
        fclose(arquivo);
    } else {
        printf("Erro ao abrir o arquivo para salvar o novo usuário.\n");
    }
    printf("Cadastro realizado com sucesso!\n");


//Função para Ordenar usuários por Email
...
int particionar(Usuario *usuario[], int baixo, int alto){
    ...
    for(int j = baixo; j < alto; j++){
        //Se o email do usuário atual for menor que o pivô
...
//Função para cadastrar um novo usuário (salvando no arquivo)
void cadastrarUsuario(Banco *banco, const char *nomeArquivo){
    ...
    //Adicionar o novo usuário ao banco e salvar no arquivo
    banco ->usuarios[banco->totalUsuarios] = novoUsuario;
    banco -> totalUsuarios++;
